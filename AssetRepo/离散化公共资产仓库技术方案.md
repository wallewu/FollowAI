...
##### **方案B：分Chunk + 索引方案（按目录分类 + 母材质聚合）**

**核心思路**：
1. 利用资产目录结构进行分类（Characters/Environment/VFX/UI）
2. 在每个分类下，按母材质聚合Shader
3. 生成独立的Shader Chunk文件和索引文件
4. MOD下载时根据依赖按需下载Chunk
...
├── /prod/tag_v1.0.3/Win64
│   ├── /shaders
│   │   ├── shader_index.json              # 索引文件（1-5MB）
│   │   └── /chunks
│   │       ├── Character.ushaderchunk     # 角色Shader（20MB）
│   │       ├── Environment.ushaderchunk   # 场景Shader（30MB）
│   │       ├── VFX.ushaderchunk          # 特效Shader（15MB）
│   │       ├── UI.ushaderchunk           # UI Shader（2MB）
│   │       └── Common.ushaderchunk       # 通用Shader（5MB）
...
```json
{
  ...
  "chunks": [
    {
      "chunk_id": "Character",
      "file": "chunks/Character.ushaderchunk",
      "size": 20971520,
      "md5": "a1b2c3d4...",
      "master_materials": [
        {
          "path": "/Content/Characters/Hero/M_Hero_Skin",
          "shader_hash": "abc123...",
          "instance_count": 50
        },
        {
          "path": "/Content/Characters/NPC/M_NPC_Cloth",
          "shader_hash": "def456...",
          "instance_count": 30
        }
      ]
    },
    {
      ...
      "file": "chunks/Environment.ushaderchunk",
      "size": 31457280,
      "md5": "e5f6g7h8...",
      "master_materials": [
        {
          "path": "/Content/Environment/Props/M_Rock",
          "shader_hash": "ghi789...",
          "instance_count": 100
        }
      ]
...
**分Chunk策略**：
```python
def classify_by_directory(asset_path):
    """根据资产路径自动分类"""
    directory_mapping = {
        ...
        '/Content/VFX': 'VFX',
        '/Content/UI': 'UI',
        '/Content/Weapons': 'Weapon',
        '/Content/Vehicles': 'Vehicle',
    }
    
    for directory, category in directory_mapping.items():
        if asset_path.startswith(directory):
            return category
    return 'Common'

def build_shader_chunks(assets):
    """构建Shader Chunk"""
    categories = {}
    
    for asset in assets:
        if isinstance(asset, UMaterial):  # 只处理母材质
            category = classify_by_directory(asset.path)
            
            if category not in categories:
                categories[category] = []
            
            categories[category].append({
                'master_material': asset.path,
                'shader_hash': asset.shader_map.hash,
                'shader_size': asset.shader_map.size,
                'instance_count': len(asset.get_instances())
            })
    ...
    for category, materials in categories.items():
        chunks[category] = {
            'chunk_id': category,
            'materials': materials,
            'total_size': sum(m['shader_size'] for m in materials)
        }
...
**处理Chunk过大的情况**：
```python
def split_large_chunk(chunk, max_size=30*1024*1024):
    """如果某个分类的Chunk超过30MB，按子目录拆分"""
    if chunk['total_size'] <= max_size:
        return [chunk]
    
    # 按子目录拆分
    # Environment.ushaderchunk (50MB) -> 拆分为：
    # - Environment_Props.ushaderchunk (20MB)
    # - Environment_Terrain.ushaderchunk (15MB)
    # - Environment_Foliage.ushaderchunk (15MB)
    pass
```

**优势**：
1. ✅ **实现简单**：直接使用项目已有的目录结构，无需额外分类规则
2. ✅ **自动分类**：新资产自动归类（根据存放目录）
3. ✅ **修改影响小**：修改一个角色材质只影响Character.ushaderchunk（20MB）
4. ✅ **冗余率低**：材质实例共享母材质Shader，跨MOD复用Chunk
5. ✅ **Chunk大小合理**：单个Chunk 2-30MB，适合网络下载

**劣势**：
1. ⚠️ **需要运行时管理**：客户端需要实现Shader Chunk下载和挂载逻辑
2. ⚠️ **索引文件维护**：每次发布需要更新shader_index.json
3. ⚠️ **资产更新影响**：修改母材质会导致整个Chunk变化，所有依赖该Chunk的MOD需要重新下载

**数据示例**（假设10万资源规模）：
```
单个MOD使用5个资产包：
- 资产数据：5 × 40MB = 200MB
- Shader Chunk：3个Chunk × 平均25MB = 75MB
- 总下载量：275MB（相比Inline减少31%）

如果玩家玩10个MOD：
- Chunk复用率：60%（不同MOD共享Chunk）
- 总下载量：10 × 200MB + 7个Chunk × 25MB = 2175MB
- 相比Inline减少64%
```
...
|------|--------------|---------------------|-------------------|
| **实现复杂度** | ⭐ 简单 | ⭐⭐⭐ 中等 | ⭐⭐⭐⭐ 复杂 |
| **单MOD下载量** | 400MB | 275MB（↓31%） | 245MB（↓39%） |
| **10个MOD下载量** | 6000MB | 2175MB（↓64%） | 3430MB（↓43%） |
| **冗余率** | 50% | 10-20% | 30-40% |
| **修改影响范围** | 整个Pak | 单个Chunk（20-30MB） | 所有依赖MOD |
| **发布效率** | ⭐⭐⭐⭐⭐ 快 | ⭐⭐⭐⭐ 快 | ⭐⭐ 慢（5-10分钟） |
| **运行时开销** | 无 | 低（Chunk管理） | 低 |
| **存储成本** | 中 | 低 | 高（每MOD一份） |
| **平台兼容性** | ⭐⭐⭐⭐⭐ 完美 | ⭐⭐⭐⭐ 好 | ⭐⭐⭐ 一般 |
...
gantt
    ...
    方案详细设计            :b1, 2026-03-01, 2w
    实现Chunk生成工具       :b2, after b1, 3w
    实现客户端Chunk管理     :b3, after b1, 4w
    索引文件生成与维护      :b4, after b2, 2w
    集成测试                :b5, after b3, 2w
    灰度发布                :b6, after b5, 2w
    全量上线                :b7, after b6, 1w
